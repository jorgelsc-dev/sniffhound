...
def build_ca(cn: str, years=10):
    ...
    subject = x509.Name(
        [
            ...
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "SniffHound"),
...
def write_env_files(out_dir: Path, master_host: str):
    ...
    (abs_dir / "master.env").write_text(
        "\n".join(
            [
                "export SNIFFHOUND_ROLE=master",
                "export SNIFFHOUND_HOST=0.0.0.0",
                "export SNIFFHOUND_PORT=45678",
                "export SNIFFHOUND_TLS_ENABLED=1",
                "export SNIFFHOUND_TLS_REQUIRE_CLIENT_CERT=0",
                f"export SNIFFHOUND_CA={ca_cert}",
                f"export SNIFFHOUND_CA_ONELINE='{ca_oneline}'",
                f"export SNIFFHOUND_TLS_CERT_FILE={master_cert}",
                f"export SNIFFHOUND_TLS_KEY_FILE={master_key}",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    (abs_dir / "agent.env").write_text(
        "\n".join(
            [
                "export SNIFFHOUND_ROLE=agent",
                f"export SNIFFHOUND_MASTER=https://{master_host}:45678",
                f"export SNIFFHOUND_CA={ca_cert}",
                f"export SNIFFHOUND_CA_ONELINE='{ca_oneline}'",
                "export SNIFFHOUND_AGENT_ID=",
                "export SNIFFHOUND_AGENT_SHARED_KEY=",
                "# Optional mTLS client certificate mode:",
                f"# export SNIFFHOUND_AGENT_CERT={agent_cert}",
                f"# export SNIFFHOUND_AGENT_KEY={agent_key}",
                "export SNIFFHOUND_IP=",
                "",
            ]
        ),
        encoding="utf-8",
    ...
    (abs_dir / "admin.env").write_text(
        "\n".join(
            [
                f"export SNIFFHOUND_CA={ca_cert}",
                f"export SNIFFHOUND_ADMIN_CERT={admin_cert}",
                f"export SNIFFHOUND_ADMIN_KEY={admin_key}",
                "",
            ]
        ),
        encoding="utf-8",
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate SniffHound CA, master, admin and agent certificates."
    )
    ...
    parser.add_argument("--ca-cn", default="SniffHound Root CA")
    parser.add_argument("--master-cn", default="sniffhound-master")
    parser.add_argument("--admin-cn", default="sniffhound-admin")
    parser.add_argument("--agent-cn", default="sniffhound-agent")
